...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    cal << "<thead>"
    if options[:show_month]
      cal << %(<tr class="#{options[:month_name_class]}"><th colspan="7">#{Date::MONTHNAMES[options[:month]]}</th></tr>)
    end    
    ...
    beginning_of_week(first, first_weekday).upto(first - 1) do |d|
      ...
      if options[:show_other_month_days]
        cal << %("><span class="mday">#{d.day}</span></td>)
      else
        cal << %("></td>)
      end    
