...
module ApplicationHelper 
  ...
  # Returns the first subscription between a given user and subscribable object. 
  ...
  def subscription(user, subscribed_object)
    case subscribed_object
    when ForumCategory
      collection = user.forum_category_subscriptions
      collection = collection.select {|sub| sub.forum_category_id == subscribed_object.id}
    when ForumTopic
      collection = user.forum_topic_subscriptions
